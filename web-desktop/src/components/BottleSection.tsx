...
const SVG_W = 120, SVG_H = 240
const BOTTLE_W = 72, BOTTLE_H = 200
const NECK_W = 28, NECK_H = 32
...
export function BottleSection(props: Props) {
  ...
  onMount(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      if (isDragging) { e.preventDefault(); handleDragMove(e.touches[0].clientY) }
    }
  ...
  return (
    <div class="flex flex-col items-center gap-2.5 w-full">
      ...
      <div class="flex flex-wrap gap-2 justify-center min-h-7">
        <For each={Array.from({ length: props.completedBottleCount() }, (_, i) => i)}>
          {() => (
            <div class="bg-blue-500/15 text-sky-400 border border-sky-400/25 rounded-[20px] text-xs font-medium py-1 px-3 flex items-center gap-1.25">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M8 2h8l1 4H7L8 2z" /><rect x="5" y="6" width="14" height="16" rx="3" />
              </svg>
              {props.size() >= 1000 ? (props.size() / 1000).toFixed(1) + 'L' : props.size() + 'ml'}
            </div>
          )}
        </For>
      </div>
      ...
      <div class="flex flex-col items-center select-none touch-none">
        <svg
          ...
          class="overflow-visible cursor-ns-resize"
          onMouseDown={(e) => { isDragging = true; handleDragMove(e.clientY) }}
          onTouchStart={(e) => { e.preventDefault(); isDragging = true; handleDragMove(e.touches[0].clientY) }}
        >
          <defs>
            {/* Clip path ensures the water fill rect stays inside the bottle body */}
            <clipPath id="wclip">
              <rect x={ORIGIN_X} y={ORIGIN_Y + BODY_TOP} width={BOTTLE_W} height={BODY_H} />
            </clipPath>
          </defs>

          {/* Bottle body outline */}
          <path d={BODY_PATH} fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.13)" stroke-width="1.5" />

          {/* Water fill — dims when near-empty to hint at the "log it" threshold */}
          <rect
            x={ORIGIN_X} y={waterSurfaceY()} width={BOTTLE_W}
            height={(ORIGIN_Y + BODY_BOT) - waterSurfaceY()}
            fill={props.fillFraction() > 0.05 ? 'rgba(56,189,248,0.42)' : 'rgba(56,189,248,0.12)'}
            clip-path="url(#wclip)"
          />

          {/* Animated water-surface ripple — hidden when full or empty */}
          <Show when={props.fillFraction() > 0.015 && props.fillFraction() < 0.99}>
            <path
              d={`M ${ORIGIN_X} ${waterSurfaceY()} q ${BOTTLE_W / 4} -3 ${BOTTLE_W / 2} 0 q ${BOTTLE_W / 4} 3 ${BOTTLE_W / 2} 0`}
              fill="none" stroke="rgba(125,211,252,0.55)" stroke-width="1.5"
            />
          </Show>

          {/* Bottle neck outline */}
          <path d={NECK_PATH} fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.13)" stroke-width="1.5" />

          {/* Bottle cap — highlights in blue when the bottle is more than half full */}
          <rect
            x={NECK_ORIGIN_X - 2} y={ORIGIN_Y - CAP_H} width={NECK_W + 4} height={CAP_H} rx="5"
            fill={props.fillFraction() > 0.5 ? 'rgba(56,189,248,0.55)' : 'rgba(255,255,255,0.12)'}
          />

          {/* Percentage label centred in the bottle body */}
          <text
            x={SVG_W / 2} y={ORIGIN_Y + BODY_TOP + BODY_H / 2 + 5}
            text-anchor="middle" font-size="15" font-weight="600"
            font-family="-apple-system, BlinkMacSystemFont, sans-serif"
            fill={props.fillFraction() > 0.18 ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.35)'}
          >
      ...
      <div class="text-[13px] text-[#7a7f96] text-center">{bottleLevelLabel()}</div>
    </div>
  )
}
