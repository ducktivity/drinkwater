...
/**
 * Computes the half-open ISO range [from, to) that spans a single local calendar day. `from` is local midnight of the given YYYY-MM-DD key and `to` is local midnight of the following day, both serialised to UTC ISO strings so the backend can compare them directly against the stored timestamps.
 */
