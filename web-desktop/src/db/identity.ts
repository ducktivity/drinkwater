...
/**
 * Client for the shared identity service (id.ducktvt.com), the suite-wide sole issuer of login codes and session tokens. It is a *different origin* from the Drinkwater app backend: passwordless login (`/auth/request`, `/auth/verify`) talks to identity directly (cross-origin, CORS-allowed), while token-authenticated calls like `/v1/auth/me`, `/v1/sync`, `/v1/logs` stay on the app backend via {@link apiClient} in api.ts.
 *
 * Unlike the app backend, identity does not emit an OpenAPI schema into this repo, so the request/response contract below is hand-written to mirror the identity DTOs (see identity/backend/api/dto.go). Keep the two in sync.
 */
...
// A separate client from the app-backend apiClient: login is pre-auth, so it needs neither the bearer-token nor the 401-logout middleware that api.ts adds.
