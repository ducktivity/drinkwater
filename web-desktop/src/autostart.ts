/**
 * Run-on-startup control for the desktop shell. Backed by Tauri's autostart
 * plugin, which registers/removes the OS login entry (the Windows registry Run
 * key). All functions are no-ops/false on the web build, where there's no OS
 * login concept to hook into.
 */
 ...
 * reliable, import-free way to gate the native-only autostart calls.
 */
